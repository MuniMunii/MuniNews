...
function NewsCard({news,tag}:{news:NewsKey[]|null|undefined,tag:statusNews|string}){
    if(tag==='all'){
        return news?.sort((a,b)=>new Date(b.updatedAt).getTime()-new Date(a.updatedAt).getTime())?.slice(0,5).map((news,index)=>{return ( 
            <Link to={`/review-news/${news.news_id}`} className={`w-full h-fit max-h-32 border border-gray-600 rounded-md text-center px-3 py-2 ${index%2==0?'dark:bg-gradient-to-b dark:from-darkTheme dark:to-sky-900':'dark:bg-gradient-to-b dark:from-darkTheme dark:to-violet-950'} `}>
                <p>{news.name_news}</p>
